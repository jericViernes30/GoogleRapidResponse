...
include("../db.php");
$confirmed = $database->getReference('confirmed')->getValue();
$dispatched = $database->getReference('history')->getValue();
?>
...
<body class="w-full flex bg-[#fffeff]">
    ...
    <div class="w-full h-screen flex flex-col gap-4 p-8">
        <div class="w-full p-4 h-1/2">
            <p class="font-semibold text-xl mb-4">Confirmed Calls</p>
            <div class="w-full flex flex-col h-5/6 overflow-y-auto">
                <div class="w-full flex items-center p-2 bg-[#bebebe]">
                    <p class="w-1/5">Name</p>
                    <p class="w-1/5">Address</p>
                    <p class="w-1/5">Contact #</p>
                    <p class="w-1/5">Call Date</p>
                    <p class="w-1/5">Call Time</p>
                </div>
                <?php
                foreach($confirmed as $key => $data){
                ?>
                <div class="w-full flex items-center p-2 border-b border-[#bebebe]">
                    <p class="w-1/5"><?php echo $data['name'] ?></p>
                    <p class="w-1/5"><?php echo $data['address'] ?></p>
                    <p class="w-1/5"><?php echo $data['number'] ?></p>
                    <p class="w-1/5"><?php echo $data['call_date'] ?></p>
                    <p class="w-1/5"><?php echo $data['call_time'] ?></p>
                </div>
                <?php } ?>
            </div>
        </div>
        <div class="w-full h-1/2 p-4">
            <p class="font-semibold text-xl mb-4">Dispatched Calls</p>
            <div class="w-full flex flex-col h-5/6 overflow-y-auto">
                <div class="w-full flex items-center p-2 bg-[#bebebe]">
                    <p class="w-1/5">Name</p>
                    <p class="w-1/5">Address</p>
                    <p class="w-1/5">Dispatched Responder</p>
                    <p class="w-1/5">Dispatched Date</p>
                    <p class="w-1/5">Dispatched Time</p>
                </div>
                <?php
                foreach($dispatched as $key => $data){
                ?>
                <div class="w-full flex items-center p-2 border-b border-[#bebebe]">
                    <p class="w-1/5"><?php echo $data['name'] ?></p>
                    <p class="w-1/5"><?php echo $data['address'] ?></p>
                    <p class="w-1/5"><?php echo $data['dispatched_responder'] ?></p>
                    <p class="w-1/5"><?php echo $data['dispatched_date'] ?></p>
                    <p class="w-1/5"><?php echo $data['dispatched_time'] ?></p>
                </div>
                <?php } ?>
            </div>
        </div>
